...
def train_model(model, optimizer,dataloader_train, dataloader_valid, n_epochs, device):
    ...
    for epoch in range(n_epochs):
        ...
        #Early stopping
        wait, best_val_loss, stop_flag = early_stop(loss_valid, best_val_loss, wait, patience=10, min_delta=0.0001)
        if stop_flag:
            break
...
def objective(trial):
    ...
    num_epochs = 100#trial.suggest_int("num_epochs", 500, 1000)
    batch_size = 32#trial.suggest_int("batch_size", 32, 128)
    #n_layers = trial.suggest_int("n_layers", 5, 15)
    weight_decay = 1e-4#trial.suggest_float("weight_decay",1e-7, 1e-5, log=True)
    #hidden_dims = []
    ...
    DATA_DIR = 'dataset\\YSYW_seq\\spec_RRI_120-32-1'
    TRAIN_RATIO = 0.8
    # npyファイルのリストを取得
